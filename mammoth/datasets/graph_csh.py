...
class Graph_CSH(Dataset):
    ...
    def create_coauth_graph(self):
        ...
        for i in self.affiliations_df.index:
            try:
                Authors_papers_links[DF_Affiliations['researcher_id'][i]].add(DF_Affiliations['doi'][i])
            except:
                Authors_papers_links[DF_Affiliations['researcher_id'][i]] = set([DF_Affiliations['doi'][i]])
                
            try:
                Papers_authors_links[DF_Affiliations['doi'][i]].add(DF_Affiliations['researcher_id'][i])                                    
            except:
                Papers_authors_links[DF_Affiliations['doi'][i]] = set([DF_Affiliations['researcher_id'][i]])
        ...
        for u in Co_authorship_links.keys():
            for v in Co_authorship_links[u].keys():
                Co_authorship_links[u][v]['weight'] = len(Co_authorship_links[u][v]['papers'])
        ...
        for u in Couthorship_network.nodes():
            Filtered_DF = self.papers_df[ self.papers_df.doi.isin(Authors_papers_links[u])]
            
            #0. Add protected attributes:
            max_year = self.papers_df.loc[Filtered_DF.year.idxmax()]['year']
            Filtered_affiliation = DF_Affiliations[(DF_Affiliations.year==max_year) & (DF_Affiliations.researcher_id==u)]
            for attribute in ['Nationality', 'aff_country', 'Gender']:
                Couthorship_network.nodes[u][attribute] = Filtered_affiliation.iloc[0][attribute]
                
                if attribute != 'Gender':
                    for category in ['Region', 'IncomeGroup']:
                        Couthorship_network.nodes[u][attribute+'_'+category] = Filtered_affiliation.iloc[0][attribute+'_'+category]
                    
            
            #1. Analyse the rankings:
            Citations = sum(Filtered_DF.N_citations)
            Productivity = len(Authors_papers_links[u])
            Degree = sum([Couthorship_network[u][v]['weight'] for v in Couthorship_network[u].keys()])
