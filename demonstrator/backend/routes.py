...
def initialize_routes(app, database):
    ...
    def new_task():
        if request.method == "POST": return handle_new_task_post(request, database)
        return handle_new_task_get(database)

    @app.route("/select_model/<int:task_id>", methods=["GET", "POST"])
    def select_model(task_id):
        if request.method == "POST": return handle_select_model_post(request, database, task_id)
    ...
    def fairness_analysis(task_id):
        if request.method == "POST": return handle_fairness_analysis_post(request, database, task_id)
    ...
    def create_variation(task_id):
        if request.method == "POST": return handle_create_variation_post(request, database, task_id)
