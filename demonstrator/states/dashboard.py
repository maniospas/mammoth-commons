...
class Dashboard(Styled):
    ...
    def edit_item(self, index):
        ...
        else: reply = QMessageBox.question(self, "Edit?",
    ...
    def showEvent(self, event): self.refresh_dashboard()
    ...
    def refresh_dashboard(self):
        ...
        for index, run in sorted_items:
            ...
            if visual_pos < len(sorted_items)-1 and run["status"] == "completed":
                ...
                if "dataset" in next_run: next_tags.append(next_run["dataset"]["module"])
                if "model" in next_run: next_tags.append(next_run["model"]["module"])
                if "analysis" in next_run: next_tags.append(next_run["analysis"]["module"])
            ...
            """)
            run_button.clicked.connect(partial(lambda i=index, r=run: self.view_result(i) if r["status"] == "completed" else self.edit_item(i)))
            ...
            if not has_same_next_tags:
                ...
                tag_container.addWidget(self.create_icon_button("🗑", "#dc3545", "Delete", partial(lambda i=index: self.delete_item(i))))
