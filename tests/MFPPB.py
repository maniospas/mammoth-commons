...
class Multi_Fair(BaseWeightBoosting, ClassifierMixin):
    ...
    def __init__(self,
                 ...
                 debug=False, 
                 X_test=None, y_test=None, preference=None, pareto=False):#,protected_attr=['Race','Sex']):
    ...
    def update_theta(self, criteria: Literal['all','fairness']='all',preference=[0.33,0.34,0.33]):
        def is_pareto(costs, maximise=False):
            """
            :param costs: An (n_points, n_costs) array
            :maximise: boolean. True for maximising, False for minimising
        ...
        best_theta, pseudo_weights = PseudoWeights(weights).do(F, return_pseudo_weights=True)
    ...
    def decision_function(self, X):
        ...
        pred = sum((estimator.predict(X)== classes).T * w  for estimator, w in zip(self.estimators_[:self.theta  ], self.estimator_alphas_[:self.theta ]))  
    ...
    def predict_proba(self, X):
        ...
        proba = sum(estimator.predict_proba(X) * w for estimator, w in zip(self.estimators_[:self.theta  ],self.estimator_alphas_[:self.theta  ]))
